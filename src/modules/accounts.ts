...
export async function getAccountsCount() {
  return await connectDb.then(async (db) => {
    const result = await db.query(
      `SELECT COUNT(DISTINCT recipient)+COUNT(DISTINCT sender) AS count FROM transactions`
    );
    return result.rows;
  });
}
...
export async function getAccountTransactionsCount(accountId: string) {
  return await connectDb.then(async (db) => {
    const result = db.query(
      `SELECT COUNT(*) AS count FROM transactions WHERE sender='${accountId}' OR recipient='${accountId}'`
    );
    return result;
  });
}
...
export async function getAccountTransactions(accountId: string) {
  return await connectDb.then(async (db) => {
    const result = db.query(
      `SELECT * FROM transactions WHERE recipient='${accountId}' OR sender='${accountId}'`
    );
    return result;
  });
}
